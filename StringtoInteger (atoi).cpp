...
were read, then the integer is 0. Change the sign as necessary (from step 2). If
...
public:
  int myAtoi(string s) {
    int i = 0;
    int sign = 1;
    long long result = 0; // Using long long to handle potential overflow
...
int i = 0; initializes a variable i to keep track of the current position in the
string.

int sign = 1; initializes a variable sign to keep track of the sign of the
result.

long long result = 0; initializes a variable result to store the converted
integer. We use long long to handle potential overflow.

The first while loop skips leading whitespaces.

The second while loop handles the main parsing of digits. It checks if the
current character is a digit using isdigit(). If it is, it updates result
accordingly.

Inside the loop, we also check for potential overflow or underflow using result
* sign > INT_MAX and result * sign < INT_MIN. If an overflow or underflow is
detected, we return the respective limit.

After the loop, we apply the sign to the result.
